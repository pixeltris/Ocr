...
namespace Cyotek.Windows.Forms
{
  ...
  /* [Designer("Cyotek.Windows.Forms.Design.ImageBoxDesigner", Cyotek.Windows.Forms.ImageBox.Design.dll, PublicKeyToken=58daa28b0b2de221")] */ public class ImageBox : VirtualScrollableControl
  {
    ...
    private bool _zoomRequiresCtrl;
    ...
    protected override void OnMouseWheel(MouseEventArgs e)
    {
        ...
        if (this.AllowZoom && this.SizeMode == ImageBoxSizeMode.Normal)
        {
            ...
            if (!ZoomRequiresCtrl || (ZoomRequiresCtrl && (ModifierKeys & Keys.Control) == Keys.Control))
            {
            ...
            if (ZoomRequiresCtrl && (ModifierKeys & Keys.Control) != Keys.Control)
            {
                for (int i = 0; i < spins; i++)
                {
                    for (int j = 0; j < 3; j++)
                    {
                        this.ProcessScrollingShortcuts(new KeyEventArgs(e.Delta > 0 ? Keys.Up : Keys.Down));
                    }
                }
            }
        }
    ...
    protected virtual void OnHorizontalMouseWheel(MouseEventArgs e)
    {
        int spins;
        ...
        for (int i = 0; i < spins; i++)
        {
            for (int j = 0; j < 3; j++)
            {
                this.ProcessScrollingShortcuts(new KeyEventArgs(e.Delta < 0 ? Keys.Left : Keys.Right));
            }
        }
    }
    ...
    protected void FireMouseHWheel(IntPtr wParam, IntPtr lParam)
    {
        ...
        OnHorizontalMouseWheel(new MouseEventArgs(MouseButtons.None, 0, x, y, tilt));
    }
    ...
    protected override void WndProc(ref Message msg)
    {
        ...
        {
            case WM_MOUSEHWHEEL:
                FireMouseHWheel(msg.WParam, msg.LParam);
                msg.Result = (IntPtr)1;
                break;
            default:
                break;
    ...
    {
        Rectangle innerRectangle = this.GetInsideViewPort();
        ...
        if (_texture != null && this.GridDisplayMode != ImageBoxGridDisplayMode.None)
        {
            switch (this.GridDisplayMode)
            {
                case ImageBoxGridDisplayMode.Image:
                    ...
                    fillRectangle = this.GetImageViewPort();
                    graphics.FillRectangle(_texture, fillRectangle);
                    break;

                case ImageBoxGridDisplayMode.Client:
                    graphics.FillRectangle(_texture, innerRectangle);
                    break;
            }
        }
    }
    ...
    [Category("Behavior")]
    [DefaultValue(true)]
    public virtual bool ZoomRequiresCtrl
    {
        get { return _zoomRequiresCtrl; }
        set
        {
            if (this.ZoomRequiresCtrl != value)
            {
                _zoomRequiresCtrl = value;
            }
        }
    }
    ...
    {
      if (this.AutoPan && !this.ViewSize.IsEmpty && (this.SelectionMode == ImageBoxSelectionMode.None || e.Button != MouseButtons.Left))
      {
